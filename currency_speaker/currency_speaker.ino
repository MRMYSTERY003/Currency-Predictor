...
int parseString(String input, String key) {
  int keyPosition = input.indexOf(key);
  if (keyPosition != -1) {
    String valueString = input.substring(keyPosition + key.length());
    int parsedValue = valueString.toInt();
    return parsedValue;
  } else {
...
void loop() {
  if(digitalRead(btn) == 0){
    analogWrite(flash, HIGH);
    espSerial.print('c');
    delay(500);
  }

  String receivedstring = espSerial.readString();


 
    int parsedValue = parseString(receivedstring, "key:");
    wifistatus = parseString(receivedstring, "Wifi:");
    if(wifistatus == 1){
